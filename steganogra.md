...
1. Choose a mode: `Visual cryptography` or `Visual steganography`.
2. Choose the pixel block size: `1x`, `2x`, or `3x`.
3. Enter text, or select an image with the `Image` button.
...
The `Center` button moves the text back to the center.
...
The files `layer1.png` and `layer2.png` are intended for printing or further work. The `result.png` file is a control preview of the overlay without offset.
...
The output is prepared as A4 at 150 DPI. It places both source layers on one page with extra cutting space between them. Each layer has registration cross marks just outside the raster and a short caption below it, for example `1752x1236 px | font 2 random | A4 150 dpi | 2026-07-11 12:34`.
...
1. Next to `Layer 1`, click `Open` and select the first image.
2. Next to `Layer 2`, click `Open` and select the second image.
3. Click `Show Overlay`.
4. Move layer 2 with the mouse or arrow keys until the result is readable.
5. Use `Save Result` to save the current overlay.
